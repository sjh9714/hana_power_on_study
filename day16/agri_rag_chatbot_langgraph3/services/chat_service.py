...
from agents.react_rag_agent import (
    create_graph
)
...
def chat(message,           # 사용자 입력 메시지
    ...
    # ==================================================
    # AUTO 와 REACT 모드 처리할 수 있게 수정합니다 
    # ==================================================
    # 
    # LangGraph 기반의 ReAct Agent를 사용하여 사용자 요청을 처리합니다.
    # ReAct 방식은 모델이 스스로 도구(Tool) 사용 여부를 판단하고 실행하며,
    # 그 결과를 바탕으로 최종 답변을 도출하는 추론 과정을 거칩니다.
    ...
    if selected_mode in ["AUTO", "REACT"]:
        ...
        # 
        # Gradio UI에 표시할 history 리스트를 업데이트합니다.
        # history 변수는 chat 함수의 인자로 들어온 리스트이며, 
        # 위에서 이미 user와 assistant(빈값) 메시지를 append 해두었습니다.
